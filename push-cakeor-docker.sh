#!/bin/bash

# EasyDarwin Docker镜像推送到Docker Hub脚本 (使用Docker命令)
# 用户: cakeor
# 项目: easydarwin

set -e

DOCKERHUB_USERNAME="cakeor"
IMAGE_NAME="easydarwin"
VERSION="v8.3.3"

echo "🚀 开始推送 EasyDarwin 镜像到 Docker Hub (使用Docker)..."
echo "用户名: $DOCKERHUB_USERNAME"
echo "镜像名: $IMAGE_NAME"
echo "版本: $VERSION"
echo ""

# 检查本地镜像是否存在
echo "📋 检查本地镜像..."
if ! docker images | grep -q "easydarwin.*latest"; then
    echo "❌ 错误: Docker中未找到 easydarwin:latest 镜像"
    echo "请先从Podman导出镜像或重新构建"
    echo "导出命令: podman save -o easydarwin.tar localhost/easydarwin:latest && docker load -i easydarwin.tar"
    exit 1
fi

# 为镜像打标签
echo "🏷️  为镜像打标签..."
docker tag easydarwin:latest $DOCKERHUB_USERNAME/$IMAGE_NAME:latest
docker tag easydarwin:latest $DOCKERHUB_USERNAME/$IMAGE_NAME:$VERSION

# 显示标签后的镜像
echo "📦 标签后的镜像列表:"
docker images | grep $IMAGE_NAME

# 登录Docker Hub
echo ""
echo "🔐 登录 Docker Hub..."
echo "请输入你的Docker Hub凭据:"
docker login

# 推送镜像
echo ""
echo "⬆️  推送镜像到 Docker Hub..."
echo "推送 latest 标签..."
docker push $DOCKERHUB_USERNAME/$IMAGE_NAME:latest

echo "推送版本标签 $VERSION..."
docker push $DOCKERHUB_USERNAME/$IMAGE_NAME:$VERSION

echo ""
echo "✅ 推送完成!"
echo ""
echo "🌐 你的镜像现在可以通过以下命令拉取:"
echo "  docker pull $DOCKERHUB_USERNAME/$IMAGE_NAME:latest"
echo "  docker pull $DOCKERHUB_USERNAME/$IMAGE_NAME:$VERSION"
echo ""
echo "🔗 Docker Hub链接:"
echo "  https://hub.docker.com/r/$DOCKERHUB_USERNAME/$IMAGE_NAME"
echo ""
echo "📋 运行容器:"
echo "  docker run -d --name easydarwin -p 8080:8080 $DOCKERHUB_USERNAME/$IMAGE_NAME:latest"